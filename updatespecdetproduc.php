...
<form action="updatespecdetproduc.php" method="POST">
<input type="checkbox" name="nf"/>num_factura <?php if(!empty($_POST['nf'])) {include('form1.php'); } ?> <br/>
<input type="checkbox" name="cvp" value="SI"/>cant_venta_p<?php if(!empty($_POST['cvp'])) {include('form2.php'); } ?><br/>
			...
			echo "<div class='alert alert-danger'>debe ingresar numero</div>";
				...
				if (!empty($f1)) 
				{
					if (!is_numeric($f1)) 
					{
						...
						$query = "update det_producto set num_factura='$f1'where cod_producto='$cp'";
				...
				if (!empty($f2)) 
				{
					if (!is_numeric($f2)) 
					{
						...
						$query = "update det_producto set cant_venta_p='$f2'where cod_producto='$cp'";
						$consul = mysqli_query($enlace,$query);
						if($consul == false){
						echo "Error al modificar datos: ".mysqli_error($enlace);
						}else{
